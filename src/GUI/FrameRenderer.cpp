...
void FrameRenderer::setInstanceId(const QString& instanceId) {
    ...
    HyperRealisticTouchEmulator::instance().setScreenSize(
        m_androidRes.width(), m_androidRes.height(), 480);
    HyperRealisticTouchEmulator::instance().setProfile(TouchProfile::NATURAL);
}
...
void FrameRenderer::dispatchTouchSequence(
    ...
    // Capture what we need — m_adb and m_androidRes are written once and
    // never changed after startup, so reading them without a lock is safe.
    AdbSocketClient* adb     = m_adb;
    QSize             screen = m_androidRes;

    QtConcurrent::run([adb, screen, points]() {
        qint64 prevTs = points.first().timestamp;

        for (const VirtualPhonePro::TouchPoint& tp : points) {
            // Honour the inter-point timing generated by the emulator.
            qint64 delay = tp.timestamp - prevTs;
            if (delay > 0 && delay < 500)   // cap at 500 ms — safety
                QThread::msleep(static_cast<unsigned long>(delay));
            prevTs = tp.timestamp;

            // Map TouchAction → scrcpy action byte
            quint8 action = 0; // ACTION_DOWN
            switch (tp.action) {
            ...
            // scrcpy InjectTouch: 28 bytes (big-endian)
            // [0]    type = 2 (InjectTouch)
            // [1]    action
            // [2-9]  pointerId (quint64)
            // [10-13] x (quint32)
            // [14-17] y (quint32)
            // [18-19] screenW (quint16)
            // [20-21] screenH (quint16)
            // [22-23] pressure (quint16, 0x0000-0xFFFF)
            // [24-27] actionButton (quint32)
            QByteArray msg(28, '\0');
            auto* p = reinterpret_cast<quint8*>(msg.data());
            p[0] = 2; // InjectTouch
            p[1] = action;

            // pointerId = 0 (single touch)
            memset(p + 2, 0, 8);

            auto bx = qToBigEndian<quint32>(static_cast<quint32>(tp.x));
            auto by = qToBigEndian<quint32>(static_cast<quint32>(tp.y));
            memcpy(p + 10, &bx, 4);
            memcpy(p + 14, &by, 4);

            auto bw = qToBigEndian<quint16>(
                          static_cast<quint16>(screen.width()));
            auto bh = qToBigEndian<quint16>(
                          static_cast<quint16>(screen.height()));
            memcpy(p + 18, &bw, 2);
            memcpy(p + 20, &bh, 2);

            // Pressure: clamp 0.0-1.0 → 0x0000-0xFFFF
            float pressureClamped = qBound(0.f, tp.pressure, 1.f);
            auto bpressure = qToBigEndian<quint16>(
                static_cast<quint16>(pressureClamped * 0xFFFF));
            memcpy(p + 22, &bpressure, 2);

            // actionButton = 1 (AMOTION_EVENT_BUTTON_PRIMARY) on DOWN, 0 on UP
            quint32 btn = (action == 0) ? 1u : 0u;
...
void FrameRenderer::mousePressEvent(QMouseEvent* e) {
    ...
    // Touch-down only — we decide tap vs drag on release
    ...
    // Send only the DOWN point immediately; UP follows in mouseReleaseEvent
...
void FrameRenderer::mouseReleaseEvent(QMouseEvent* e) {
    ...
    if (dist < 20.f && holdMs < 300) {
        // Short press in place → realistic tap (DOWN already sent, add UP)
        TouchPoint up = te.generateTouchPoint(0, a.x(), a.y(), TouchAction::UP);
        up.pressure  = 0.f;
        // Delay UP by realistic tap-hold duration
        up.timestamp = QDateTime::currentMSecsSinceEpoch()
                       + static_cast<qint64>(holdMs);
        seq.append(up);

    } else if (holdMs >= 300 && dist < 20.f) {
        // Long press → generate hold MOVE points then UP
        seq = te.generateLongPress(a.x(), a.y(),
                                   static_cast<int>(holdMs));
        // Remove first DOWN (already sent) — keep MOVEs and final UP
        if (!seq.isEmpty() && seq.first().action == TouchAction::DOWN)
            seq.removeFirst();

    } else {
        // Drag/swipe — determine dominant direction
...
void FrameRenderer::mouseMoveEvent(QMouseEvent* e) {
    // Hover moves without button held — ignore
    if (!(e->buttons() & Qt::LeftButton)) return;

    // Live MOVE points while dragging — send at reduced rate (every 16 ms ≈ 60 Hz)
    qint64 now = QDateTime::currentMSecsSinceEpoch();
