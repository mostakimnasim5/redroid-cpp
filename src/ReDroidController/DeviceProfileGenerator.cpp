...
void DeviceTemplateDatabase::initializeTemplates() {
    ...
    m_templates.push_back(CertifiedDeviceTemplate{
        ...
        .android_version = 14,
        .sdk_version = 34,
        .security_patch = []{ auto t=std::time(nullptr); auto tm=*std::localtime(&t); char buf[11]; tm.tm_mday=1; tm.tm_mon=(tm.tm_mon==0?11:tm.tm_mon-1); if(tm.tm_mon==11)tm.tm_year--; std::strftime(buf,sizeof(buf),"%Y-%m-%d",&tm); return std::string(buf); }(),
