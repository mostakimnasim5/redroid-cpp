...
bool NetworkProfile::fromJson(const QJsonObject& json) {
    ...
    tcpFingerprint.windowSize = tcpJson.value("windowSize").toInt();
    tcpFingerprint.mss = tcpJson.value("mss").toInt();
    tcpFingerprint.windowScaling = tcpJson.value("windowScaling").toBool();
    tcpFingerprint.sackOk = tcpJson.value("sackOk").toInt();
    tcpFingerprint.timestamp = tcpJson.value("timestamp").toInt();
    tcpFingerprint.selectiveAck = tcpJson.value("selectiveAck").toInt();
    ...
    for (auto it = telephonyObj.constBegin(); it != telephonyObj.constEnd(); ++it) {
        telephonyProperties[it.key()] = it.value().toString();
    }
